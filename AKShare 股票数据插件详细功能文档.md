...
### 3.1 东方财富网-行情报价-指定股票
**功能**: 获取指定股票的实时行情报价数据，包括五档买卖盘口、成交量、价格等详细的交易信息，用于分析股票的买卖盘情况和市场深度。
...
### 7.16 东方财富网-资产负债表-按报告期-指定股票
**功能**: 获取指定股票的资产负债表数据，按报告期维度提供详细的资产、负债和股东权益信息。

**参数输入**: 需要输入股票代码，格式为"SH600519"等带市场标识的代码。
...
### 7.17 东方财富网-资产负债表-按年度-指定股票
**功能**: 获取指定股票的资产负债表数据，按年度维度提供详细的资产、负债和股东权益信息。
...
### 7.18 东方财富网-利润表-按报告期-指定股票
**功能**: 获取指定股票的利润表数据，按报告期维度提供详细的收入、成本和利润信息。
...
### 7.19 东方财富网-利润表-按年度-指定股票
**功能**: 获取指定股票的利润表数据，按年度维度提供详细的收入、成本和利润信息。
...
### 7.20 东方财富网-利润表-按单季度-指定股票
**功能**: 获取指定股票的利润表数据，按单季度维度提供详细的收入、成本和利润信息。
...
### 7.21 东方财富网-现金流量表-按报告期-指定股票
**功能**: 获取指定股票的现金流量表数据，按报告期维度提供详细的现金流入流出信息。
...
### 7.22 东方财富网-现金流量表-按年度-指定股票
**功能**: 获取指定股票的现金流量表数据，按年度维度提供详细的现金流入流出信息。
...
### 7.23 东方财富网-现金流量表-按单季度-指定股票
**功能**: 获取指定股票的现金流量表数据，按单季度维度提供详细的现金流入流出信息。
...
### 7.24 东方财富网-同行比较-成长性比较-指定股票
**功能**: 获取指定股票的成长性比较数据，与同行业其他公司进行成长性指标对比分析。
...
### 7.25 东方财富网-同行比较-估值比较-指定股票
**功能**: 获取指定股票的估值比较数据，与同行业其他公司进行估值指标对比分析。
...
### 7.26 东方财富网-同行比较-杜邦分析比较-指定股票
**功能**: 获取指定股票的杜邦分析比较数据，与同行业其他公司进行杜邦分析指标对比。
...
**目标地址**: 待补充
**描述**: 东方财富网-同行比较-杜邦分析比较
**限量**: 单次获取指定股票的杜邦分析比较数据
...
### 7.27 东方财富网-同行比较-公司规模-指定股票
**功能**: 获取指定股票的公司规模比较数据，与同行业其他公司进行规模指标对比分析。
...
### 11.4 东方财富网-美股-知名美股的实时行情数据-指定类别
...
### 11.6 新浪-美股-历史行情数据-指定股票代码、复权方式
**功能**: 获取美股历史行情数据，支持前复权、后复权、不复权等多种复权方式，历史数据按日频率更新。

**参数输入**: 需要输入美股代码和复权方式(qfq前复权/hfq后复权/空字符串不复权)。
