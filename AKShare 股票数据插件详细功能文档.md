...
> **文档说明**: 本文档是 AKShare 股票数据插件的详细功能文档，包含所有12个工具共156个接口的完整技术说明。

---

## 📋 文档目录

- [工具1：股票市场总貌 (25个接口)](#工具1股票市场总貌-stock-market-summary)
- [工具2：个股信息总貌 (25个接口)](#工具2个股信息总貌-individual-stock-info-summary)
...
- [工具6：资金流向分析 (14个接口)](#工具6资金流向分析-capital-flow-analysis)
- [工具7：股票财务数据分析 (23个接口)](#工具7股票财务数据分析-stock-financial-data-analysis)
- [工具8：股票技术分析 (13个接口)](#工具8股票技术分析-stock-technical-analysis)
- [工具9：个股综合技术指标 (5个接口)](#工具9个股综合技术指标-stock-comprehensive-technical-indicators) ⭐ **扩展指标工具**
- [工具10：港股数据 (14个接口)](#工具10港股数据-stock-hk-data) ⭐ **独立工具**
- [工具11：美股数据 (8个接口)](#工具11美股数据-stock-us-data) ⭐ **独立工具**
- [工具12：股票指数数据 (9个接口)](#工具12股票指数数据-stock-index-data) ⭐ **指数分析工具**
...
### 2.12 东方财富网-个股-十大流通股东-指定股票、日期(季末)
...
### 2.21 新浪财经-股票评级记录-指定股票
**功能**: 获取指定股票的新浪财经股票评级记录数据，包括评级日期、评级机构、投资评级、目标价位、评级说明等详细信息，显示所有历史数据。

**参数输入**: 需要输入股票代码，支持A股6位数字代码格式。

**引用信息**:
- **AKSHARE 接口**: `stock_institute_recommend_detail`
- **目标地址**: https://vip.stock.finance.sina.com.cn/q/go.php/vIR_RatingNew/kind/rzrq/index.phtml
- **描述**: 新浪财经-机构评级-股票评级记录
- **限量**: 单次获取所有历史数据
...
### 2.22 东方财富网-估值分析-指定股票
**功能**: 获取指定股票的东方财富网估值分析数据，包括最新价、涨跌幅、市盈率、市净率、市销率、市现率、PEG、总市值、流通市值、净资产、净利润、营业收入、每股收益、每股净资产、每股营业收入、每股经营现金流、每股现金流、每股公积金、每股未分配利润、ROE、ROA、毛利率、净利率、资产负债率、总股本、流通股、流通A股、限售A股、限售股、总资产、总负债、货币资金、应收账款、存货、流动资产、固定资产、无形资产、流动负债、非流动负债、总负债、股东权益合计、营业收入同比增长、净利润同比增长、净资产同比增长、总资产同比增长、营业收入TTM、净利润TTM、净资产TTM、总资产TTM、每股收益TTM、每股净资产TTM、每股营业收入TTM、每股经营现金流TTM、每股现金流TTM、每股公积金TTM、每股未分配利润TTM、ROE_TTM、ROA_TTM、毛利率TTM、净利率TTM、资产负债率TTM、总股本TTM、流通股TTM、流通A股TTM、限售A股TTM、限售股TTM、总资产TTM、总负债TTM、货币资金TTM、应收账款TTM、存货TTM、流动资产TTM、固定资产TTM、无形资产TTM、流动负债TTM、非流动负债TTM、总负债TTM、股东权益合计TTM、营业收入同比增长TTM、净利润同比增长TTM、净资产同比增长TTM、总资产同比增长TTM等详细估值指标。

**参数输入**: 需要输入股票代码，支持A股6位数字代码格式。

**引用信息**:
- **AKSHARE 接口**: `stock_value_em`
- **目标地址**: https://emweb.securities.eastmoney.com/PC_HSF10/CompanySurvey/Index?type=web&code=SH688686#gsgk-0
- **描述**: 东方财富网-个股-估值分析
- **限量**: 单次获取指定 symbol 的所有数据
...
### 2.23 新浪财经-新股发行-指定股票
**功能**: 获取指定股票的新股发行信息，包括发行价格、发行数量、募集资金等IPO相关数据。

**参数输入**: 需要输入股票代码，支持A股6位数字代码格式。

**引用信息**:
- **AKSHARE 接口**: `stock_ipo_info`
- **目标地址**: https://vip.stock.finance.sina.com.cn/corp/go.php/vISSUE_NewStock/stockid/300059.phtml
- **描述**: 新浪财经-新股发行
- **限量**: 单次获取所有历史数据
...
### 2.24 新浪财经-股票增发-指定股票
**功能**: 获取指定股票的增发信息，包括增发价格、增发数量、募集资金等增发相关数据。

**参数输入**: 需要输入股票代码，支持A股6位数字代码格式。

**引用信息**:
- **AKSHARE 接口**: `stock_add_stock`
- **目标地址**: https://vip.stock.finance.sina.com.cn/corp/go.php/vISSUE_AddStock/stockid/300059.phtml
- **描述**: 新浪财经-股票增发
- **限量**: 单次获取所有历史数据
...
### 2.25 新浪财经-限售解禁-指定股票
**功能**: 获取指定股票的限售解禁信息，包括解禁日期、解禁数量、解禁市值等限售解禁相关数据。

**参数输入**: 需要输入股票代码，支持A股6位数字代码格式。

**引用信息**:
- **AKSHARE 接口**: `stock_restricted_release_queue_sina`
- **目标地址**: https://vip.stock.finance.sina.com.cn/corp/go.php/vISSUE_Shareholder/stockid/300059.phtml
- **描述**: 新浪财经-限售解禁
- **限量**: 单次获取所有历史数据
...
### 3.1 东方财富网-行情报价-指定股票
**功能**: 获取东方财富网指定股票的实时行情报价数据，包括买一卖一价格、盘口深度、实时成交等逐笔行情信息。

**参数输入**: 需要输入股票代码参数，支持A股6位数字代码格式，用于获取该股票的实时报价数据。

**引用信息**:
- **AKSHARE 接口**: `stock_bid_ask_em`
- **目标地址**: https://push2.eastmoney.com/api/qt/stock/details/get
- **描述**: 东方财富网-行情报价-指定股票
- **限量**: 单次返回指定 symbol 的行情报价数据
...
**功能**: 获取上海证券交易所A股市场的实时行情数据，包括股价、涨跌幅、成交量、成交额、市盈率等交易信息，用于实时监控沪A股票的交易状况。
...
### 6.4 同花顺-资金流向-大单追踪
**功能**: 获取大额资金流向追踪数据，包括大单买入、卖出的详细信息，用于分析大资金的动向。

**参数输入**: 此接口无需输入任何参数，直接调用即可获取大单追踪数据。

**引用信息**:
**AKSHARE 接口**: `stock_fund_flow_big_deal`
**目标地址**: http://data.10jqka.com.cn/funds/ddzjl/#refCountId=data_55f13c2c_254
...
### 6.8 东方财富网-板块资金流-排名-指定统计周期、板块类型
...
### 6.10 东方财富网-行业资金流-xx行业个股资金流-指定行业名称、统计周期
...
### 7.9 东方财富网-资产负债表-按报告期-指定股票
**功能**: 获取指定股票按报告期的资产负债表数据，包括资产、负债、所有者权益等完整的资产负债表信息。

**参数输入**: 需要输入股票代码，用于获取该公司的资产负债表数据。

**引用信息**:
- **AKSHARE 接口**: `stock_balance_sheet_by_report_em`
- **目标地址**: http://data.eastmoney.com/bbsj/
- **描述**: 东方财富网-资产负债表-按报告期
...
### 7.10 东方财富网-资产负债表-按年度-指定股票
**功能**: 获取指定股票按年度的资产负债表数据，用于年度财务对比分析。

**参数输入**: 需要输入股票代码，用于获取该公司的年度资产负债表数据。
...
### 7.11 东方财富网-利润表-按报告期-指定股票
**功能**: 获取指定股票按报告期的利润表数据，包括营业收入、营业成本、净利润等完整的利润表信息。

**参数输入**: 需要输入股票代码，用于获取该公司的利润表数据。
...
### 7.12 东方财富网-利润表-按年度-指定股票
**功能**: 获取指定股票按年度的利润表数据，用于年度财务对比分析。

**参数输入**: 需要输入股票代码，用于获取该公司的年度利润表数据。
...
### 7.13 东方财富网-利润表-按单季度-指定股票
**功能**: 获取指定股票按单季度的利润表数据，用于季度财务分析。

**参数输入**: 需要输入股票代码，用于获取该公司的季度利润表数据。
...
### 7.14 东方财富网-现金流量表-按报告期-指定股票
**功能**: 获取指定股票按报告期的现金流量表数据，包括经营活动、投资活动、筹资活动的现金流量信息。

**参数输入**: 需要输入股票代码，用于获取该公司的现金流量表数据。
...
### 7.15 东方财富网-现金流量表-按年度-指定股票
**功能**: 获取指定股票按年度的现金流量表数据，用于年度财务对比分析。

**参数输入**: 需要输入股票代码，用于获取该公司的年度现金流量表数据。
...
### 7.16 东方财富网-现金流量表-按单季度-指定股票
**功能**: 获取指定股票按单季度的现金流量表数据，用于季度财务分析。

**参数输入**: 需要输入股票代码，用于获取该公司的季度现金流量表数据。
...
### 7.17 同花顺-资产负债表-指定股票、报告类型
**功能**: 获取指定股票的资产负债表数据，包括完整的资产负债和所有者权益信息。

**参数输入**: 需要输入股票代码和报告类型（年报/季报/中报）。
...
|-----|-----|-----|
| 报告期 | object | - |
| 完整的资产负债表指标 | 多种类型 | 包含所有资产负债和所有者权益科目 |

### 7.18 同花顺-个股利润表-指定股票、报告类型
**功能**: 获取指定股票的利润表数据，包括完整的收入和利润信息。
...
|-----|-----|-----|
| 报告期 | object | - |
| 完整的利润表指标 | 多种类型 | 包含所有收入和利润科目 |

### 7.19 同花顺-现金流量表-指定股票、报告类型
**功能**: 获取指定股票的现金流量表数据，包括完整的现金流入和流出信息。
...
|-----|-----|-----|
| 报告期 | object | - |
| 完整的现金流量表指标 | 多种类型 | 包含所有现金流入流出科目 |

### 7.20 新浪财经-财务报表-关键指标-指定股票
...
### 7.22 东方财富网-A股财务分析-主要指标-指定股票、报告类型
...
### 10.13 东方财富网-港股-个股-成长性对比-指定股票代码
...
### 10.14 东方财富网-港股-财务分析指标-指定股票代码、指标类型
...
### 10.15 东方财富网-港股-三大财务报表-指定股票代码、报表类型、指标类型
...
### 11.2 新浪-美股-实时行情(延15分钟)
**功能**: 获取美股实时行情数据的备用接口，提供与11.1相同的功能，用于数据源备份和稳定性保障。
...
### 11.3 东方财富网-美股-知名美股的实时行情数据-指定类别
...
### 11.4 新浪-美股-历史行情数据-指定股票代码、复权方式
**功能**: 获取新浪财经提供的美股历史行情数据，可选择返回复权后数据，提供从上市以来的完整历史数据，包括前复权、后复权因子。

**参数输入**: 需要输入美股代码和复权方式（不复权/""/前复权/qfq/后复权/hfq/前复权因子/qfq-factor/后复权因子/hfq-factor）。

**引用信息**:
**AKSHARE 接口**: `stock_us_daily`
**目标地址**: https://stock.finance.sina.com.cn/usstock/quotes/AAPL.html
**描述**: 新浪财经-美股-历史行情数据
**限量**: 单次获取指定 symbol 的所有历史数据
...
### 11.5 东方财富网-美股-历史行情数据-指定股票代码、周期、日期范围、复权方式
...
### 11.6 东方财富网-美股-每日分时行情-指定股票代码
**功能**: 获取美股的分时历史行情数据，用于美股短期技术分析和日内交易策略制定。

**参数输入**: 需要输入美股代码和分时周期(1/5/15/30/60分钟)。接口会自动获取最近5天的分时数据。
...
### 11.7 东方财富网-美股-财务分析指标-指定股票代码、指标类型
...
### 11.8 东方财富网-美股-三大财务报表-指定股票代码、报表类型、指标类型
...
## 工具12：股票指数数据 (Stock Index Data) ⭐ **指数分析工具**

### 12.1 新浪财经-A股指数-实时行情
**功能**: 获取A股市场所有指数的实时行情数据，包括上证指数、深证成指、创业板指等主要指数的实时价格、涨跌幅、成交量等信息。

**参数输入**: 此接口无需输入任何参数，直接调用即可获取所有A股指数的实时行情数据。

**引用信息**:
- **AKSHARE 接口**: `stock_zh_index_spot_sina`
...
### 12.2 东方财富-A股指数-实时行情-按类别
**功能**: 获取A股指数按类别分类的实时行情数据，包括沪深重要指数、上证系列指数、深证系列指数、指数成份、中证系列指数等不同类别的指数数据。

**参数输入**: 需要选择指数类别(沪深重要指数/上证系列指数/深证系列指数/指数成份/中证系列指数)。
...
### 12.3 新浪财经-A股指数-历史行情数据-指定指数、日期范围过滤
**功能**: 获取指定A股指数的历史行情数据，提供从上市以来的完整历史数据，包括开盘价、收盘价、最高价、最低价、成交量等信息。

**参数输入**: 需要输入指数代码(如sz399552)，格式为市场标识+指数代码。
...
### 12.4 腾讯-A股指数-历史行情数据-指定指数、日期范围过滤
**功能**: 获取指定A股指数的历史行情数据，提供腾讯财经数据源的历史数据，包括完整的OHLCV数据。

**参数输入**: 需要输入指数代码(如sh000919)，格式为市场标识+指数代码。

**引用信息**:
- **AKSHARE 接口**: `stock_zh_index_daily_tx`
- **目标地址**: https://gu.qq.com/sh000919/zs
- **描述**: 股票指数(或者股票)历史行情数据
...
### 12.5 东方财富-A股指数-历史行情数据-指定指数、日期范围
**功能**: 获取指定A股指数的历史行情数据，支持多种市场标识格式，提供东方财富数据源的历史数据。

**参数输入**: 需要输入指数代码(如sz399552)、开始日期(如19900101)、结束日期(如20500101)。
...
### 12.6 东方财富-A股指数-历史行情数据-通用-指定指数、周期、日期范围
**功能**: 获取指定A股指数的历史行情数据，支持日线、周线、月线等多种周期，提供更灵活的数据查询方式。

**参数输入**: 需要输入指数代码(如399282)、周期(daily/weekly/monthly)、开始日期(如19700101)、结束日期(如22220101)。
...
### 12.7 东方财富-A股指数-分时行情数据-指定指数、分钟周期、日期时间范围
**功能**: 获取指定A股指数的分时行情数据，支持1分钟、5分钟、15分钟、30分钟、60分钟等多种分钟级周期，用于短期技术分析。

**参数输入**: 需要输入指数代码(如000001)、分钟周期(1/5/15/30/60)、开始日期时间(如2025-10-08 09:15:00)、结束日期时间(如2025-10-30 15:00:00)。

**引用信息**:
- **AKSHARE 接口**: `index_zh_a_hist_min_em`
- **目标地址**: https://quote.eastmoney.com/center/hszs.html
- **描述**: 东方财富网-中国股票指数-分时行情数据
- **限量**: 单次返回指定指数指定分钟周期的分时数据
...
### 12.8 指数趋势动量震荡指标-指定指数、日期范围
**功能**: 计算指定指数的趋势动量震荡指标，包括MACD、RSI、KDJ等技术指标，用于指数技术分析和趋势判断。

**参数输入**: 需要输入指数代码(如sz399552)、开始日期(如20250101)、结束日期(如20251231)。

**引用信息**:
- **AKSHARE 接口**: `index_trend_momentum_oscillator`
- **目标地址**: 内部计算接口
- **描述**: 指数趋势动量震荡指标
- **限量**: 单次返回指定指数的趋势动量震荡指标
...
### 12.9 指数趋势动量震荡指标(分钟)-指定指数、周期(分钟)、日期时间范围
**功能**: 计算指定指数的分钟级趋势动量震荡指标，提供更精细的技术分析数据，用于短期交易决策和日内分析。

**参数输入**: 需要输入指数代码(如000001)、分钟周期(1/5/15/30/60)、开始日期时间(如2025-10-08 09:15:00)、结束日期时间(如2025-10-30 15:00:00)。

**引用信息**:
- **AKSHARE 接口**: `index_trend_momentum_oscillator_minute`
- **目标地址**: 内部计算接口
- **描述**: 指数趋势动量震荡指标(分钟级)
- **限量**: 单次返回指定指数的分钟级趋势动量震荡指标
...
| 涨跌额 | float64 | - |
| 换手率 | float64 | 注意单位: % |
| MA5 | float64 | 5周期移动平均线 |
| MA10 | float64 | 10周期移动平均线 |
| MA20 | float64 | 20周期移动平均线 |
| MA30 | float64 | 30周期移动平均线 |
| MA60 | float64 | 60周期移动平均线 |
| RSI6 | float64 | 6周期相对强弱指标 |
| RSI12 | float64 | 12周期相对强弱指标 |
| RSI24 | float64 | 24周期相对强弱指标 |
| MACD | float64 | MACD指标 |
| MACD_SIGNAL | float64 | MACD信号线 |
| MACD_HIST | float64 | MACD柱状图 |
| KDJ_K | float64 | KDJ随机指标K值 |
...
| BOLL_MIDDLE | float64 | 布林带中轨 |
| BOLL_LOWER | float64 | 布林带下轨 |
| VMA5 | float64 | 5周期成交量均线 |
| VMA10 | float64 | 10周期成交量均线 |
| VMA20 | float64 | 20周期成交量均线 |

---

| 工具编号 | 工具名称 | 接口数量 | 主要功能 |
|----------|----------|----------|----------|
| 工具1 | 股票市场总貌 | 25个 | 市场概况、风险监控、交易提醒 |
| 工具2 | 个股信息总貌 | 25个 | 基本面信息、财务摘要、股东分析 |
| 工具3 | 股票实时行情 | 15个 | 多市场实时行情、比价分析 |
| 工具4 | 股票历史行情 | 12个 | 历史数据、技术分析基础 |
| 工具5 | 沪深港通持股 | 12个 | 外资动向、跨境资金、汇率数据 |
| 工具6 | 资金流向分析 | 14个 | 资金面分析、主力追踪 |
| 工具7 | 股票财务数据分析 | 23个 | 财务报表、财务指标 |
| 工具8 | 股票技术分析 | 13个 | 技术指标、ESG评级 |
| 工具9 | 个股综合技术指标 | 5个 | 技术分析、量化研究 ⭐ **扩展指标工具** |
| 工具10 | 港股数据 | 14个 | 港股投资分析、跨市场比较 ⭐ **独立工具** |
| 工具11 | 美股数据 | 8个 | 美股投资分析、全球市场研究 ⭐ **独立工具** |
| 工具12 | 股票指数数据 | 9个 | 指数分析、市场趋势 ⭐ **指数分析工具** |
| **总计** | **12个工具** | **175个接口** | **全方位股票数据分析** |
