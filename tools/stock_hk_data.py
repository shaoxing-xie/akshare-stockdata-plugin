from collections.abc import Generator
from typing import Any
import pandas as pd

import akshare as ak
from provider.akshare_stockdata import safe_ak_call, build_error_payload
from provider.akshare_registry import get_interface_config
from dify_plugin import Tool
from dify_plugin.entities.tool import ToolInvokeMessage
from .common_utils import process_dataframe_output, process_other_output, handle_empty_result, handle_akshare_error


class StockHkDataTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        import logging
        try:
            logging.info(f"StockHkDataTool received parameters: {tool_parameters}")
            
            interface = tool_parameters.get("interface", "")
            logging.info(f"Interface: {interface}")
            
            if not interface:
                yield self.create_text_message("请选择要调用的接口")
                yield self.create_json_message({"error": "interface required", "received_params": tool_parameters})
                return
        except Exception as e:
            logging.error(f"Error in _invoke start: {e}")
            yield self.create_text_message(f"参数处理错误: {e}")
            yield self.create_json_message({"error": f"parameter processing error: {e}"})
            return
        
        # 定义港股接口配置
        interface_configs = {
            # 港股实时行情接口
            "stock_hk_spot_em": {
                "fn": ak.stock_hk_spot_em,
                "description": "港股-实时行情(延15分钟)"
            },
            "stock_hk_spot": {
                "fn": ak.stock_hk_spot,
                "description": "新浪-港股-实时行情(延15分钟)"
            },
            "stock_hk_main_board_spot_em": {
                "fn": ak.stock_hk_main_board_spot_em,
                "description": "港股主板-实时行情(延15分钟)"
            },
            "stock_hk_famous_spot_em": {
                "fn": ak.stock_hk_famous_spot_em,
                "description": "知名港股实时行情数据"
            },
            # 港股历史行情接口
            "stock_hk_hist": {
                "fn": ak.stock_hk_hist,
                "requires_symbol": True,
                "requires_period": True,
                "requires_date_range": True,
                "requires_adjust": True,
                "description": "港股-历史行情数据-指定股票代码、周期、日期范围、复权方式"
            },
            "stock_hk_hist_min_em": {
                "fn": ak.stock_hk_hist_min_em,
                "requires_symbol": True,
                "requires_period": True,
                "requires_date_range": True,
                "requires_adjust": True,
                "description": "港股-每日分时行情-指定股票代码、周期、日期范围、复权方式"
            },
            "stock_hk_daily": {
                "fn": ak.stock_hk_daily,
                "requires_symbol": True,
                "requires_adjust": True,
                "description": "新浪-港股-历史行情数据-指定股票代码、复权方式"
            },
            # 港股个股信息接口
            "stock_hk_security_profile_em": {
                "fn": ak.stock_hk_security_profile_em,
                "requires_symbol": True,
                "description": "港股-个股-证券资料-指定股票代码"
            },
            "stock_hk_company_profile_em": {
                "fn": ak.stock_hk_company_profile_em,
                "requires_symbol": True,
                "description": "港股-个股-公司资料-指定股票代码"
            },
            "stock_hk_fhpx_detail_ths": {
                "fn": ak.stock_hk_fhpx_detail_ths,
                "requires_symbol": True,
                "description": "港股-个股-分红派息-指定股票代码"
            },
            "stock_hk_financial_indicator_em": {
                "fn": ak.stock_hk_financial_indicator_em,
                "requires_symbol": True,
                "description": "东方财富网-港股-个股-财务指标-指定股票代码"
            },
            "stock_hk_dividend_payout_em": {
                "fn": ak.stock_hk_dividend_payout_em,
                "requires_symbol": True,
                "description": "东方财富网-港股-个股-分红派息-指定股票代码"
            },
            "stock_hk_growth_comparison_em": {
                "fn": ak.stock_hk_growth_comparison_em,
                "requires_symbol": True,
                "description": "东方财富网-港股-个股-成长性对比-指定股票代码"
            },
            # 港股财务分析接口
            "stock_financial_hk_analysis_indicator_em": {
                "fn": ak.stock_financial_hk_analysis_indicator_em,
                "requires_symbol": True,
                "requires_indicator_hk": True,
                "timeout": 600,
                "description": "港股-财务分析-主要指标-指定股票代码、指标类型"
            },
            "stock_financial_hk_report_em": {
                "fn": ak.stock_financial_hk_report_em,
                "requires_symbol": True,
                "requires_report_type": True,
                "requires_indicator_hk": True,
                "timeout": 600,
                "param_mapping": {
                    "stock": "symbol",
                    "symbol": "report_type", 
                    "indicator": "indicator_hk"
                },
                "description": "港股-财务报表-三大报表-指定股票代码、报表类型、指标类型"
            },
            # 沪深港通资金流向接口
            "stock_hsgt_fund_flow_summary_em": {
                "fn": ak.stock_hsgt_fund_flow_summary_em,
                "description": "沪深港通资金流向"
            }
        }
        
        # 获取接口配置
        config = interface_configs.get(interface)
        if not config:
            yield self.create_text_message(f"不支持的接口: {interface}")
            yield self.create_json_message({"error": f"unsupported interface: {interface}"})
            return
        
        try:
            # 构建调用参数
            call_params = {}
            
            # 处理股票代码
            if config.get("requires_symbol"):
                symbol = tool_parameters.get("symbol", "")
                if not symbol:
                    yield self.create_text_message("此接口需要港股代码参数")
                    yield self.create_json_message({"error": "symbol required for this interface"})
                    return
                
                # 港股代码格式转换
                # 根据原来版本的处理逻辑
                if symbol.startswith("HK"):
                    # 去掉HK前缀，如HK00700 -> 00700
                    symbol = symbol[2:]
                
                # 根据接口类型进行不同的格式转换
                if interface in ["stock_hk_fhpx_detail_ths"]:
                    # 分红派息接口需要4位格式：00700 -> 0700
                    if symbol.isdigit() and len(symbol) == 5 and symbol.startswith('0'):
                        symbol = symbol[1:]  # 移除开头的0
                elif interface in ["stock_hk_hist", "stock_hk_hist_min_em", "stock_financial_hk_analysis_indicator_em", "stock_financial_hk_report_em"]:
                    # 历史数据接口和财务分析接口保持5位格式，不进行转换
                    pass
                # 其他接口保持原始格式
                
                call_params["symbol"] = symbol
            
            # 处理周期参数
            if config.get("requires_period"):
                if interface in ["stock_hk_hist"]:
                    period = tool_parameters.get("period", "daily")
                    call_params["period"] = period
                elif interface in ["stock_hk_hist_min_em"]:
                    period = tool_parameters.get("period2", "5")
                    call_params["period"] = period
            
            # 处理日期范围
            if config.get("requires_date_range"):
                start_date = tool_parameters.get("start_date", "20250101")
                end_date = tool_parameters.get("end_date", "20250915")
                call_params["start_date"] = start_date
                call_params["end_date"] = end_date
            
            # 处理复权方式
            if config.get("requires_adjust"):
                adjust = tool_parameters.get("adjust", "none")
                # 将"none"转换为空字符串，因为AKShare接口需要空字符串表示不复权
                if adjust == "none":
                    adjust = ""
                call_params["adjust"] = adjust
            
            # 处理指标参数
            if config.get("requires_indicator"):
                indicator = tool_parameters.get("indicator_hk", "年度")
                call_params["indicator"] = indicator
            
            # 处理报表类型
            if config.get("requires_report_type"):
                report_type = tool_parameters.get("report_type", "资产负债表")
                call_params["report_type"] = report_type
            
            # 处理参数映射（如果存在）
            param_mapping = config.get("param_mapping", {})
            if param_mapping:
                # 清空之前的参数，使用映射系统
                call_params = {}
                for ak_param, tool_param in param_mapping.items():
                    if tool_param == "symbol" and symbol:
                        call_params[ak_param] = symbol
                    elif tool_param == "report_type" and report_type:
                        call_params[ak_param] = report_type
                    elif tool_param == "indicator":
                        # 根据接口类型选择正确的indicator参数
                        if config.get("requires_indicator_hk", False) and indicator:
                            call_params[ak_param] = indicator
            
            # 获取重试次数和超时时间
            retries = tool_parameters.get("retries", 5)
            timeout = tool_parameters.get("timeout", 600)
            
            # 使用接口特定的超时时间
            interface_timeout = config.get("timeout", timeout)
            
            # 调用接口
            result = safe_ak_call(
                config["fn"],
                retries=retries,
                timeout=interface_timeout,
                **call_params
            )
            
            if result is None:
                yield self.create_text_message("接口调用失败，未返回数据")
                yield self.create_json_message({"error": "interface call failed, no data returned"})
                return
            
            # 处理结果
            if isinstance(result, pd.DataFrame):
                if result.empty:
                    yield self.create_text_message("接口返回空数据")
                    yield self.create_json_message({"data": [], "message": "empty result"})
                    return
                
                # 处理DataFrame输出
                yield from process_dataframe_output(result, self)
            else:
                # 处理其他类型输出
                yield from process_other_output(result, self)
                
        except Exception as e:
            yield from handle_akshare_error(e, self, f"接口: {interface}")
